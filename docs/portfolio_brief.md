...
- 소비자 점검 기준 비교: 공식 금융소비자보호 안내를 바탕으로 직접 작성한 확인 기준과 질문을 보여줍니다.
...
- 근거 중심 결과: 위험 문장 안에서 감지된 단어를 색상으로 표시하고, `확인 필요`·`주의 후보` 상태, 쉬운 이유, 질문, 공식 출처·기준일·원문 링크·적용 이유를 한 화면에 제공합니다.
- 접근성 입력: 약관 사진 OCR, 로컬 음성 입력, 브라우저 읽어주기를 지원해 직접 입력이 어려운 사용자의 진입 장벽을 낮춥니다.
- 오탐 방지 평가: 약관 20개와 상담 문구 20개, 총 40개 문장으로 위험 탐지 성공률과 정상 문장 오탐률을 자동 측정합니다.

## 공식 데이터 활용

현재 앱에 import된 공식 데이터는 12종이며, 모두 공공데이터포털에서 `이용허락범위 제한 없음`을 확인했습니다.
...
상업적 이용·변경이 금지된 뉴스 메타데이터와 이용조건이 불명확한 PDF·표준약관 원문 추출본은 실행 데이터 및 제출 배포물에서 제외했습니다. 표준약관은 원문을 재배포하지 않고 공식 페이지 링크와 프로젝트가 직접 작성한 소비자 점검 기준만 제공합니다.
...
- Validation: pytest, ESLint, TypeScript, Next.js production build
...
flowchart TD
  ...
  B --> C["위험 탐지 규칙"]
  B --> D["공식 데이터 검색"]
  D --> E["금융용어, FAQ, 통계, 모범상담 사례"]
  C --> F["안전점수, 쉬운 설명, 질문 카드 생성"]
  E --> F
  F --> G["Next.js 고령층 UX 화면"]
...
- 현재 구현: 사진 OCR, 음성 입력, 브라우저 읽어주기, 보호자 요약과 단계형 행동 카드를 제공합니다.

### 기술적 완성도

- 기술 적정성: 이미지 OCR, 텍스트/PDF/HWPX/CSV 기반 공식 데이터 키워드 검색, 규칙 기반 탐지를 결합했습니다.
...
- 실행 데이터는 이용허락범위 제한 없음이 확인된 자료로 제한했습니다.
- 공공누리 제4유형과 이용조건 미확인 자료는 실행·배포·검색 대상에서 제외했습니다.
- 데이터별 출처, 원문 URL, 기준일, 행 수, 이용조건을 레지스트리와 제3자 고지에 기록했습니다.
- 문서 10MB·PDF 60쪽·입력 글자 수 제한과 보안 응답 헤더를 적용했습니다.
- 문서·음성은 현재 요청에서만 처리하고 데이터베이스에 저장하지 않는 원칙을 명시했습니다.
- 기관 도입 시 보유기간, 파기 절차, 처리위탁사, 담당자 정보를 실제 운영환경에 맞춰 확정해야 합니다.

## 한계와 개선 계획

- 현재 판단은 법률 판단이 아니라 소비자 보호 보조 정보입니다. 실제 민원 제출 전 사실관계 확인이 필요합니다.
- 현재 공식 근거 검색은 키워드 기반이며, 향후 벡터 검색과 LLM 기반 쉬운 말 재작성을 추가할 수 있습니다.
